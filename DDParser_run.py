...
def visualize(parser_result, img_id=0):
    """
    Visualize the dependency.
    parser_result = [{'word': list[str],　'head': list[int],　'deprel': list[str]}, {},...]
    ...
    """
    word, head, deprel = parser_result['word'],parser_result['head'],parser_result['deprel']
    ...
    for i in range(len(nodes)):
        ...
        if i == 0:
            ...
            ax.annotate(
                ...
                textcoords='data',
                fontproperties=font_manager.FontProperties(fname="/home/liupei/anaconda3/envs/paddlepaddle/lib/python3.7/site-packages/SourceHanSansCN-Regular.otf"))
            # draw the curve
            ax.annotate(
                "",
                xy=xy,
    ...
    cv2.imwrite('/home/liupei/桌面/test{}.jpg'.format(img_id), data)
...
class DDParser(object):
    ...
    def parse(self, inputs):
        ...
        print(predicts)
        for i, each_parser_result in enumerate(outputs):
            visualize(each_parser_result, i)
        return outputs
